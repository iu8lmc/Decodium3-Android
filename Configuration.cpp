...
public:
  ...
  explicit FrequencyDialog (IARURegions * regions_model, Modes * modes_model, QWidget * parent = nullptr)
    : QDialog {parent}
  {
    start_date_time_edit_ = new QDateTimeEdit(QDateTime(QDate::currentDate(), QTime(0,0,0,0), QTimeZone::UTC), parent);
    end_date_time_edit_ = new QDateTimeEdit(QDateTime(QDate::currentDate().addDays(2), QTime(0,0,0,0), QTimeZone::UTC), parent);

    enable_dates_checkbox_ = new QCheckBox {tr ("")};
    start_date_time_edit_->setDisplayFormat("yyyy.MM.dd hh:mm:ss 'UTC'");
    start_date_time_edit_->setTimeZone(QTimeZone::UTC);
    ...
    end_date_time_edit_->setTimeZone(QTimeZone::UTC);
  ...
  if (device_name.size ())
    {
      ...
      auto const& devices = (mode == QAudioDevice::Input) ? QMediaDevices::audioInputs () : QMediaDevices::audioOutputs ();
      Q_FOREACH (auto const& p, devices)
        {
          // qDebug () << "Configuration::impl::find_audio_device: input:" << (QAudioDevice::Input == mode) << "name:" << p.description ();
          if (p.description () == device_name)
            {
              ...
              for (int ch = p.minimumChannelCount (); ch <= p.maximumChannelCount (); ++ch)
                channel_counts << ch;
...
void Configuration::impl::load_audio_devices(QAudioDevice::Mode mode, QComboBox * combo_box, QAudioDevice * device)
{
    ...
    {
#ifndef WIN32
        if (hideCARD_ && p.description().contains("CARD=")) {
        ...
        device_items.emplace_back(p.description(), info);
    }
    ...
    for (int i = 0; i < static_cast<int>(device_items.size()); ++i)
    {
        ...
        if (device->description() == item.first)
        {
            current_index = i;
...
void Configuration::impl::fill_port_combo_box(QComboBox* cb)
{
    ...
    std::sort(ports.begin(), ports.end(), [](const QSerialPortInfo& a, const QSerialPortInfo& b) {
        bool isANumeric = a.portName().mid(3).toInt();
        bool isBNumeric = b.portName().mid(3).toInt();
        if (isANumeric && isBNumeric)
        {
            return a.portName().mid(3).toInt() < b.portName().mid(3).toInt();
        }
        else if (isANumeric)
        {
