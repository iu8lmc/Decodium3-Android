subroutine subtractft2(dd,itone,f0,dt)

! Subtract an ft2 signal
!
...
! Subtract         : dd(t)    = dd(t) - 2*REAL{cref*cfilt}
  ...
  logical first
  data first/.true./
  common/heap2/cref(NFRAME),camp(NMAX),cfilt(NMAX),cw(NMAX),xjunk(NFRAME)
  save first
  ...
  bt=1.0
  nss=NSPS
  call gen_ft2wave(itone,nsym,nss,fs,f0,cref,xjunk,icmplx,NFRAME)
  camp=0.
  do i=1,nframe
    id=nstart-1+i
