        ...
        if (Qt::DisplayRole == role)
          {
            if (model_index.column () == fieldIndex ("frequency"))
              ...
              {                     // adjust date/time to Qt format
                QLocale locale;
                value = locale.toString (QDateTime::fromMSecsSinceEpoch (value.toULongLong () * 1000ull, QTimeZone::UTC), locale.dateFormat (QLocale::ShortFormat) + " hh:mm:ss");
              }
          }
      }
    return value;
  ...
  while (m_->export_query_.next ())
    {
      ...
      stream << QString {"QSO: %1 DG %2 %3 %4 %5 %6\n"}
                      ...
                      .arg (QDateTime::fromMSecsSinceEpoch (m_->export_query_.value (when_index).toULongLong () * 1000ull, QTimeZone::UTC).toString ("yyyy-MM-dd hhmm"))
                      .arg (my_call, -12)
