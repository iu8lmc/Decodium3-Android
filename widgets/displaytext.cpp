...
void DisplayText::AudioAlerts()
{
  ...
  if(m_config->alert_Enabled()) {
        ...
        // removed - set by setSampleFormat
        QAudioSink* audio;
  ...
  if(m_config->alert_Enabled()) {
        ...
        case 0:
            if (play_MyCall) {
                ...
                audio->start(effect2);
#else
                { auto *se = new QSoundEffect(this); se->setSource(QUrl::fromLocalFile(binPath + "/sounds/MyCall.wav")); se->play(); connect(se, &QSoundEffect::playingChanged, se, &QObject::deleteLater); }  // for Linux and macOS
#endif
        ...
        case 1:
            if (play_DXCC) {
                ...
                audio->start(effect3);
#else
                { auto *se = new QSoundEffect(this); se->setSource(QUrl::fromLocalFile(binPath + "/sounds/DXCC.wav")); se->play(); connect(se, &QSoundEffect::playingChanged, se, &QObject::deleteLater); }  // for Linux and macOS
#endif
        ...
        case 2:
            if (play_DXCCOB && !play_DXCC) {
                ...
                audio->start(effect4);
#else
                { auto *se = new QSoundEffect(this); se->setSource(QUrl::fromLocalFile(binPath + "/sounds/DXCCOnBand.wav")); se->play(); connect(se, &QSoundEffect::playingChanged, se, &QObject::deleteLater); }  // for Linux and macOS
#endif
        ...
        case 3:
            if (play_Continent) {
                ...
                audio->start(effect5);
#else
                { auto *se = new QSoundEffect(this); se->setSource(QUrl::fromLocalFile(binPath + "/sounds/Continent.wav")); se->play(); connect(se, &QSoundEffect::playingChanged, se, &QObject::deleteLater); }  // for Linux and macOS
#endif
        ...
        case 4:
            if (play_ContinentOB && !play_Continent) {
                ...
                audio->start(effect6);
#else
                { auto *se = new QSoundEffect(this); se->setSource(QUrl::fromLocalFile(binPath + "/sounds/ContinentOnBand.wav")); se->play(); connect(se, &QSoundEffect::playingChanged, se, &QObject::deleteLater); }  // for Linux and macOS
#endif
                play_ContinentOB = false;
                play_GridOB = false;
                play_CQZOB = false;
        ...
        case 5:
            if (play_CQZ) {
                ...
                audio->start(effect7);
#else
                { auto *se = new QSoundEffect(this); se->setSource(QUrl::fromLocalFile(binPath + "/sounds/CQZone.wav")); se->play(); connect(se, &QSoundEffect::playingChanged, se, &QObject::deleteLater); }  // for Linux and macOS
#endif
                play_CQZ = false;
                play_CQZOB = false;
        ...
        case 6:
            if (play_CQZOB && !play_CQZ) {
                ...
                audio->start(effect8);
#else
                { auto *se = new QSoundEffect(this); se->setSource(QUrl::fromLocalFile(binPath + "/sounds/CQZoneOnBand.wav")); se->play(); connect(se, &QSoundEffect::playingChanged, se, &QObject::deleteLater); }  // for Linux and macOS
#endif
                play_CQZOB = false;
                alertsTimer.start (1800);
                startIndex = nextStartIndex;
        ...
        case 7:
            if (play_ITUZ) {
                ...
                audio->start(effect9);
#else
                { auto *se = new QSoundEffect(this); se->setSource(QUrl::fromLocalFile(binPath + "/sounds/ITUZone.wav")); se->play(); connect(se, &QSoundEffect::playingChanged, se, &QObject::deleteLater); }  // for Linux and macOS
#endif
                play_ITUZ = false;
                play_ITUZOB = false;
                play_GridOB = false;
...
#else
                { auto *se = new QSoundEffect(this); se->setSource(QUrl::fromLocalFile(binPath + "/sounds/ITUZoneOnBand.wav")); se->play(); connect(se, &QSoundEffect::playingChanged, se, &QObject::deleteLater); }  // for Linux and macOS
#endif
                play_ITUZOB = false;
                play_GridOB = false;
...
#else
                { auto *se = new QSoundEffect(this); se->setSource(QUrl::fromLocalFile(binPath + "/sounds/Grid.wav")); se->play(); connect(se, &QSoundEffect::playingChanged, se, &QObject::deleteLater); }  // for Linux and macOS
#endif
                play_Grid = false;
                play_GridOB = false;
                alertsTimer.start (1000);
...
#else
                { auto *se = new QSoundEffect(this); se->setSource(QUrl::fromLocalFile(binPath + "/sounds/GridOnBand.wav")); se->play(); connect(se, &QSoundEffect::playingChanged, se, &QObject::deleteLater); }  // for Linux and macOS
#endif
                play_GridOB = false;
                alertsTimer.start (1500);
                startIndex = nextStartIndex;
...
#else
                { auto *se = new QSoundEffect(this); se->setSource(QUrl::fromLocalFile(binPath + "/sounds/CQ.wav")); se->play(); connect(se, &QSoundEffect::playingChanged, se, &QObject::deleteLater); }  // for Linux and macOS
#endif
                play_CQ = false;
                alertsTimer.start (1000);
                nextStartIndex++;
